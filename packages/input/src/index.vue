<template>
  ...
  >
    ...
    />
    <div
      class="qib-box-options el-select-dropdown"
    ...
    >
      <div v-if="!loading && options.length">
        <i
          ...
          :data-quick-value="opt[optionValue]"
          :key="opt[optionValue]"
...
export default {
  ...
  methods: {
    ...
    selectOnFocus(e) {
      e.target.select();
      if (this.selectable) this.toggleSuggestionsPopper();
    },
    scrollIntoView(el) {
      el.offsetParent.scrollTop = Math.max(
        0,
        el.offsetTop - el.offsetParent.clientHeight + el.offsetHeight * 1.25
      );
    },
    keydown(e) {
  ...
  computed: {
    icon: {
      ...
      get() {
        ...
        if (this.focus && this.clearable && this.hiddenValue)
          return "circle-close";
  ...
  watch: {
    ...
    hiddenValue(value, old) {
      if (
        old === value ||
        (this.valueDetectOnChanged &&
          value == this.value) /*来自属性value绑定新值时不触发事件*/
      )
        return;
      ...
      this.$emit(
        "change",
        value,
        this.options.find(_ => String(_[this.optionValue]) == value),
        this.scope
      );
        ...
        .then(response => {
          ...
          if (errorCode === 0) {
            if (typeof data.data == "string") {
              ...
              this.displayText = data.data[this.optionLabel];
            } else {
              this.clear();
            }
          } else {
